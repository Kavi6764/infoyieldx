...
const {
  hashedPassword,
  ComparePassword,
} = require("../Auth/PasswordHashed.js");
...
router.post("/signup", async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ id: newHR._id }, "mySecretKey123", {
      expiresIn: "1h",
    });
...
router.post("/add-employee", ProfileImage.single("image"), async (req, res) => {
  ...
  try {
    ...
    const avatar = req.file
      ? `localhost:5000/profile/${req.file.filename}`
      : "";
...
router.put("/update-employee/:id", ProfileImage.single("image"),async (req, res) => {
    try {
      ...
      const updated = await Employee.findByIdAndUpdate(id, updatedData, {
        new: true,
      });
...
router.post("/add-blogs", Blogupload.single("image"), async (req, res) => {
  ...
  try {
    const blog = new Blog({
      title,
      content,
      imageUrl: imageUrl ? `localhost:5000/blogs/${imageUrl}` : "",
      department,
...
router.post(
  ...
  async (req, res) => {
    const { title, description, projectUrl, technologies, status ,department} = req.body;
    const imageUrl = req.file?.filename;
    try {
      const portfolio = new Portfolio({
        ...
        description,
        projectUrl,
        imageUrl: `localhost:5000/portfolio/${imageUrl}`,
        technologies,
        department
      });
      ...
      res
        .status(500)
        .json({ error: "Failed to add portfolio", details: err.message });
    }
  }
);
...
router.delete("/delete-portfolio/:id", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to delete portfolio", details: err.message });
  }
});
...
router.put( "/update-portfolio/:id",Portfolioupload.single("image"),async (req, res) => {
    ...
    try {
      const updatedPortfolio = await PortfolioModel.findByIdAndUpdate(
        req.params.id,
        { title, description, projectUrl, technologies, status ,department},
        { new: true }
      );
      if (!updatedPortfolio)
        return res.status(404).json({ error: "Portfolio not found" });
